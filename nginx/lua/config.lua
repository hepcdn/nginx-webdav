local ngx = require("ngx")

local Config = {
    data = {
        -- Note: env vars also need to be listed in nginx/conf.d/default.main

        -- Read VERSION env variable (which is set during the dockerfile build)
        server_version = os.getenv("VERSION") or "unknown",
        -- Read SERVER_ADDRESS generated by docker-entrypoint.sh
        server_address = os.getenv("SERVER_ADDRESS") or "unknown",

        -- These should never need to change
        -- They are also hardcoded in confi.d/include/locations.conf
        local_path = "/var/www/webdav",
        uriprefix = "/webdav",

        -- Federation settings
        seed_peers = "", -- comma separated list of seed peers
        gossip_delay = 10, -- in seconds
        gossip_fraction = 0.1, -- fraction of peers to gossip to

        -- This is used in webdav_write_content and webdav_tpc_content
        receive_buffer_size = 1024*1024,
        -- How often to send a performance marker (in seconds)
        performance_marker_timeout = 5,

        -- This is used in webdav_tpc_content
        tpc_redirect_limit = 5,

        -- This is used in cksumutil
        checksum_block_size = 64*1024*1024,

        -- discovery = "https://cms-auth.cern.ch/.well-known/openid-configuration",
        openidc_iss = "https://cms-auth.cern.ch/",
        openidc_pubkey = [[-----BEGIN PUBLIC KEY-----

LwIDAQAB
-----END PUBLIC KEY-----]],
        openidc_client_id = "",
        openidc_client_secret = "",

       -- To be able to confirm that the test server is the same one we spoke
       -- to, have the health check reply this ID. So the test sets the ID and
       -- the service replies with the same number
       health_check_id = -1,
    }
}

 function Config.load(path)
    -- Update the Config object with the values from the file at the given path
    local cjson = require("cjson")

    local f = io.open(path, "r")
    if f == nil then
       return nil
    end
    local content = f:read("*a")
    f:close()
    local newvalues = cjson.decode(content)
    for k,v in pairs(Config.data) do
        if k == "server_version" or k == "server_address" then
            -- Don't overwrite the server version and address
        elseif newvalues[k] ~= nil then
            Config.data[k] = newvalues[k]
        end
    end
 end

 return Config
